...
class User extends Controller
{
            ...
            if ($UserResult == "" || $UserResult == null) {
                ...
                if ($UserResult['password'] == md5($data['password'])) {
                    ...
                    if ($ipaddress['code'] == 400) {
                        $emailcontent = "<h3>您的账号在异地登录,登录IP为" . Common::get_user_ip(). "(" .$ipaddress['msg'] . "),如是你本人登录，请忽略，不是请立即修改密码</h3>";
    ...
    public function ResetPassword(Request $request)
    {
        ...
        if ($updateuser > 0) {
            try {
                $emailresult = Db::name('email')->where('id', 1)->find();
                $emailcontent = "<h3>您的随机密码是：". $newpassword.",如是你本人操作，请忽略!<br>请及时修改为您易记的密码</h3>";
