...
class User extends Controller
{
            ...
            if ($UserResult == "" || $UserResult == null) {
                ...
                if ($UserResult['password'] == md5($data['password'])) {
                    ...
                    if ($ipaddress['code'] == 400) {
                        $emailcontent = "<h3>你的账号在异地登录,登录IP为" . Common::get_user_ip(). "(" .$ipaddress['msg'] . "),如是你本人登录，请忽略，不是请立即修改密码</h3>";
    ...
    public function GetPasswordCode(Request $request)
    {
        ...
        if ($ckcodetime['ip'] == Common::get_user_ip()) {
            if (time() - $ckcodetime['creattime'] < 60) {
                ...
                try {
                    $emailcontent = "<h3>你的找回密码验证码是：" . $passcode .",如是你本人操作，请忽略!</h3>";
            ...
            if ($user == null || $user == "") {
                ...
                if ($ckcodetime['ip'] == Common::get_user_ip()) {
                    if (time() - $ckcodetime['creat_time'] > 60) {
                        ...
                        try {
                            $emailcontent = "<h3>你的注册验证码是：". $emailcode.",如是你本人操作，请忽略!</h3>";
    ...
    public function ResetPassword(Request $request)
    {
        ...
        if ($updateuser > 0) {
            try {
                $emailresult = Db::name('email')->where('id', 1)->find();
                $emailcontent = "<h3>你的随机密码是：". $newpassword.",如是你本人操作，请忽略!</h3>";
